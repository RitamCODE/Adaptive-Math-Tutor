...
4. `route_after_engagement` is where the retry ladder actually lives: 4 consecutive signal-bearing wrong answers anywhere → `end_session`; wrong with attempts left → `retry_problem` (same problem stays on screen); wrong on the third attempt → `demote_skill` (re-serves a problem on the prerequisite); correct and mastery crossed 0.8 → `advance_skill`; correct otherwise → `new_problem`; quest length or 2-skills-mastered reached → `end_session`.
5. Every branch terminates at `END`. `api.py` stores whatever `SessionState` comes out and reshapes it into `AnswerResponse` — `skill_mastery` and `misconception_log` never reach the client directly, only a derived `skill_progress` list and a per-answer `Feedback` object, and `correct_answer` is withheld from `Feedback` until either the answer is right or attempt 3 is reached (constraint #7).
...
backend/
  ...
  models/
    ...
    bkt.py                 BKTParams, MASTERY_THRESHOLD (0.8), update_mastery()
  nodes/
