...
## Digit-width ladder — `skills/_difficulty_ladder.py`

Operand digit-width (whether a problem is `7 + 0` or `405 + 314`) is **not** derived from BKT mastery. It used to be — `_difficulty_for()` fed `state.skill_mastery` straight into `bucket()` below — but since one correct answer already lifts a fresh skill from 0.30 to ~0.90 (see the mastery section above), that jumped straight from the "easy"/1-digit bucket to "hard"/3-digit, skipping "medium"/2-digit entirely. Digit-width is now its own sustained-run ladder per skill, tracked in `SessionState.digit_level`/`digit_level_run`/`seen_combos`, and it neither reads nor writes `skill_mastery` or `mastery_run`.

`WIDTH_LADDER` gives each skill's own tier sequence: `addition_no_carry`/`subtraction_no_borrow` run `[1, 2, 3]`; `addition_carry`/`subtraction_borrow` run `[2, 3]`, since a carry or borrow needs at least two digits to exist. `graph.py`'s `_difficulty_for()` maps the current tier index to a representative float per skill's own ladder length so `bucket()` (unchanged, still `easy`/`medium`/`hard`) resolves to the intended width without any skill's `_WIDTH_BY_BUCKET` table needing to change.

`advance_digit_level()`, called from `update_mastery_node` on every signal-bearing answer (unconditionally — unlike the BKT update, it is not skipped for `digit_reversal`), runs two different gates:

- **General case** (any tier above a skill's narrowest): 3 consecutive correct answers (`LEVEL_UP_RUN`) advance to the next tier; a wrong answer resets the run to zero.
- **Narrowest tier**, only for `addition_no_carry`/`subtraction_no_borrow` (`NARROWEST_TIER_SKILLS`): 2 consecutive correct answers (`NARROW_TIER_BASE_RUN`) advance, but the pair must escalate — a "trivial" combo first (an operand is 0, or for subtraction the subtrahend is 0 or the operands are equal), then a "non-trivial" one. `pick_combo()` draws from `all_combos()` (every valid 1-digit pair for that skill) avoiding whatever's already in `seen_combos[skill]`, falling back to a repeat only once the relevant bucket is genuinely exhausted — coverage-without-boredom is a selection heuristic, not an advancement gate.

Digit-width never goes back down. Sustained struggle is still handled entirely by the existing attempt-3 demotion to the prerequisite skill, which this ladder has no part in.
...
`_arithmetic.py` holds the shared pure helpers every generator uses: digit-width helpers, `has_any_carry`/`has_any_borrow` (used both to build problems that require carrying/borrowing and to build ones that deliberately don't), `format_question`/`parse_operands` (the canonical `"a op b"` string and its inverse — any future flavor-text wrapper must preserve this round-trip), and `bucket(difficulty)` mapping a float to `easy`/`medium`/`hard` template widths — the float itself now comes from the digit-width ladder above, not from raw mastery. `addition_no_carry.generate()`/`subtraction_no_borrow.generate()` additionally accept an optional `combo_hint` kwarg (the ladder's `pick_combo()` result) so the "easy" bucket can request a specific trivial/non-trivial/unseen 1-digit pair instead of drawing at random.
