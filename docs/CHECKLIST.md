...
- [ ] A demoted skill comes back once, after two correct answers on the prerequisite; failing it again ends the quest instead of demoting further. — ready to test (2026-09-15): new `pending_resurface`/`resurface_progress`/`resurfaced_skills` fields on `SessionState`, a `resurface_skill` graph node, and 6 new tests in `test_retry_ladder.py`; not yet confirmed live in-browser (needs a played-out `struggling`-style session that fails a skill, then answers its prerequisite correctly twice).
...
- [ ] Verify the fading actually changes between the `new` and `fluent` seeds. — ready to test (2026-09-14): `?seed=` is now built (see section J); `new` lands on `addition_no_carry` at the default `0.3` mastery (ten-frame scaffold, open band) and `fluent` lands on `addition_carry` at `0.85` (abstract-only band). Verified live in-browser.
