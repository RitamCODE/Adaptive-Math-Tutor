...
### G. Manipulatives — ready to test for all three: column arithmetic with base-ten blocks (addition-with-carrying and subtraction-with-borrowing), number line (the four `number_line/*` misconceptions), and ten-frame (`addition_no_carry`, opt-in only — that skill has no bug rules, so nothing can auto-open it). As of 2026-09-16 all three are remediation rather than default furniture: none appears on attempt 1, and the mastery band decides what a wrong answer delivers. The blocks widget was rebuilt the same day as `ColumnArithmetic` — the stacked equation and the blocks are now one grid — so every row below wants a fresh look rather than a re-read of the previous note. Fastest route to subtraction: `?seed=borrowing`.

- [ ] Column layout: the two numbers are stacked in standard column form and each block column sits directly under its own digit column, at every problem width. Scroll the page with the blocks open — the equation stays pinned. — ready to test (2026-09-16): verified live in-browser on `72 - 29`, `71 - 47`, `598 + 271`.
- [ ] Borrowing is something the student causes and watches: a short column shows a labelled pad, the lending column is marked, and dragging one block onto the pad breaks it into ten with the same glow-and-snap as carrying, while the written digits pick up the standard struck-through regrouping marks. — ready to test (2026-09-16): verified live in-browser with a real pointer drag on `32 - 28` and `71 - 47`.
- [ ] Borrowing across a zero takes two visible steps (hundreds into tens, then tens into ones) and never reaches past the empty column. — ready to test (2026-09-16): the generator produces this shape in roughly 6% of 3-digit problems, so it is slow to reach by play. The board logic was swept over all 426,355 problems the curriculum can generate (`columnBoard.js`, chained borrows included) and every one solves correctly; the *visual* two-step still wants a human look.
- [ ] Taking away is an action on objects: each unit to remove is its own slot, a slot fills and keeps the piece that landed in it, and a running "n of m taken" is visible. Nothing disappears without a trace. — ready to test (2026-09-16). This is what made removals previously read as something the app had done for the student.
- [ ] Each column's answer digit appears under the rule only when that column is finished, no digit appears for a column the student has not worked, and nothing anywhere shows a running total mid-drag. Leading zeros are not written. — ready to test (2026-09-16): verified live; see the per-column exception in CLAUDE.md's "A manipulative never resolves the answer for the student".

- [ ] `86 + 94` can be solved end to end by dragging, with no keyboard. — ready to test (2026-09-16): re-verified live in-browser after the column rebuild on `598 + 271` (carry out of the ones, resolved to 869) and on subtraction `32 - 28` with a borrow. The earlier note that subtraction had only ever been checked "via a temporary local harness" is exactly why the borrow flow shipped as a dead end; `?seed=borrowing` now puts it one click away, and it wants a real play-through on every future change.
- [ ] Ten loose sticks cannot remain loose: they glow, snap into a bundle and slide to the tens column, and the carry lands in the column to its left with the little "1" appearing over it. There is deliberately **no** running numeric total on screen — only the per-column answer digits, each one appearing when its own column is finished. — ready to test (2026-09-16): verified live in-browser on `598 + 271`, where the tens column overflowed and bundled.
...
- [ ] Full session playable at 1024 x 768 with nothing clipped or off-screen, including no horizontal scrolling with the widest problem the generator can produce (`999 + 999`, nine tokens in every place of both operands) and the blocks open. — **needs a fresh look (2026-09-16)**: the column rebuild changed how width is allocated. Both grids are capped at `--ca-cols * --ca-track-max` and otherwise share the width equally, so nothing overflows horizontally (confirmed: no page-level sideways scroll), but a four-column board — only `999 + 999` once its carry reaches the thousands — narrows each column to about two tokens per row and grows tall. The equation is sticky, so it stays readable; the question is whether the height is acceptable, and that is a human judgement.
